...
import proc_ifix, proc_data11, plot_funcs1
...
def get_Data1(tickers_selected, tipo=""):
    ...
    if len(error_message) > 0:
        ...
        st.sidebar.warning('A lista ' + msg + ' foi retirada por possuir menos de 30 dados')
...
def check_Email(email):
    ...
    reg_ex = r'[^@]+@[^@]+\.[^@]+'   # expressao imperfeita mas que funciona
...
with col2:
    ...
    <div class="title">
        ...
        <h3>3 - Selecione os Ativos da sua Carteira a partir da lista suspensa (clique a seta). </h3>
...
if e_mail:
    ...
    # escreve o resultado no arquivo final de e-mails
...
if e_mail:
    ...
    if tipo_ativo == 'Ações':
        op_List = gera_Lista(path='tmp/', name_file='all_B3', type_file='csv', codbdi=2)  # usando st.cache
    ...
    if tipo_ativo:
        ...
        if options:
            ...
            if _OK:
                ...
                df = get_Data1(options, param_tipo)
                df1 = df.copy()
                ...
                if (len(df) < 30):
                    ...
                    with col2:
                        ...
                        # col2.dataframe(df_disp.style.highlight_max(axis=0))
                        st.write("  ")
                        st.markdown(''' ### **Ativos do Portfólio** ''')
                        plot_funcs1.view_Table2(df_disp)
                        #plot_funcs1.v_Table3(df_disp)

                    if tipo_ativo == "FIIs":
                        df_acum_c_IFIX = proc_ifix.incl_Ifix(df)  # inclui o IFIX para ser plotado junto com o s ativos
                    else:
                ...
                ##### PORTFOLIO DE VARIÂNCIA MÍNIMA (Menor Risco)
                        ...
                        # (3) Obtém os PESOS do portfólio de Mín. Volatilidade e da fronteira eficiente correspondente
                        pesos_Min_Vol, fronteira_eficiente_Min_Vol = proc_data11.port_Min_Vol(covar, retorno_medio)

                        # (4) ALOCAÇÃO MÍN. VOLATILIDADE - Obtém os percentuais de alocacao do portfólio com Mín. Volatilidade, para um valor de patrimônio
                        #     default = 100000
                        alocacao_Min_Vol, sobra_Min_Vol = proc_data11.aloc_Otima(df1, pesos_Min_Vol, valor=100000)

                        # (5) CÁLCULO DA PERFORMANCE MÍN. VOLATILIDADE - Mede a performance do portfólio com Mín. Volatilidade
                        ...
                        st.markdown(''' ### **Quantidades de Ações/Cotas para o Portfólio de Variância Mínima** ''')
