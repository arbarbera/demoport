...
class EfficientFrontier(object):
    ...
    def minimum_volatility(self, save_weights=True):
        ...
        self.last_optimisation = "Volatilidade Mín."
        # set optimal weights
        if save_weights:
    ...
    def maximum_sharpe_ratio(self, save_weights=True):
        ...
        # if successful, set self.last_optimisation
        self.last_optimisation = "Máx. Índice de Sharpe"
        # set optimal weights
        if save_weights:
            self.weights = result["x"]
            self.df_weights = self._dataframe_weights(self.weights)
    ...
    def efficient_return(self, target, save_weights=True):
        ...
        # if successful, set self.last_optimisation
        self.last_optimisation = "Retorno Eficiente"
        # set optimal weights
        if save_weights:
            self.weights = result["x"]
            self.df_weights = self._dataframe_weights(self.weights)
    ...
    def efficient_volatility(self, target):
        ...
        # if successful, set self.last_optimisation
        self.last_optimisation = "Volatilidade Eficiente"
        # set optimal weights
        self.weights = result["x"]
        self.df_weights = self._dataframe_weights(self.weights)
        return self.df_weights
    ...
    def plot_optimal_portfolios(self):
        ...
        plt.scatter(
            ...
            color="g",
            s=450,
            label="Fronteira de mín. Volatilidade",
        )
        plt.scatter(
            max_sharpe_vals[0],
            max_sharpe_vals[1],
            ...
            color="r",
            s=450,
            label="Fronteira de máx. Índice de Sharpe",
        )
        plt.legend()
    ...
    def properties(self, verbose=False):
        ...
        if verbose:
            string = "-" * 70
            string += "\nPortfólio otimizado para {}".format(self.last_optimisation)
            string += "\n\nTime window/frequência: {}".format(self.freq)
            string += "\nTaxa Livre de Risco: {}".format(self.risk_free_rate)
            string += "\nRetorno Anual Esperado: {:.3f}".format(expected_return)
            string += "\nVolatilidade Anual: {:.3f}".format(volatility)
            string += "\nÍndice de Sharpe: {:.3f}".format(sharpe)
            string += "\n\nPesos Ótimos:"
