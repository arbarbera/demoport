...
- a public class ``Portfolio`` that holds and calculates quantities of a financial
  portfolio, which is a collection of Stock instances.re
...
from ef_frontier import EfficientFrontier
from montecarlo import MonteCarloOpt
...
class Portfolio(object):
    ...
    def properties(self):
        ...
        string += "\nAções: {}".format(", ".join(stocknames))
        string += "\nTime window/frequência: {}".format(self.freq)
        string += "\nTaxa Livre de Risco: {}".format(self.risk_free_rate)
        string += "\nRetorno Esperado: {:0.3f}".format(self.expected_return)
        string += "\nVolatilidade: {:0.3f}".format(self.volatility)
        string += "\nÍndice de Sharpe: {:0.3f}".format(self.sharpe)
        string += "\n\nViés(Skewness):"
    ...
    def __str__(self):
        # print short description
        string = "Contém informações sobre o portfólio."
        return string
