...
class MonteCarloOpt(MonteCarlo):
    ...
    def properties(self):
        ...
        for val in opt_vals:
            string += "-" * 70
            string += "\nPortfólio Otimizado para {}".format(
                val.replace("Min", "Minimum").replace("Max", "Maximum")
            )
            string += "\n\nPeríodo: {} days".format(self.freq)
            string += "\nRetorno esperado: {0:0.3f}".format(
                self.opt_results.loc[val]["Expected Return"]
            )
            string += "\nVolatilidade: {:0.3f}".format(
                self.opt_results.loc[val]["Volatility"]
            )
            string += "\nÍndice de Sharpe: {:0.3f}".format(
